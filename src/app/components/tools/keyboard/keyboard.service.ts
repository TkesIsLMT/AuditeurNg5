...
import { KEYBOARD_LAYOUTS } from './keyboard-layouts.config';
...
export class KeyboardService {
   ...
   */
  private _keyboardRefAtThisLevel: KeyboardRef<KeyboardComponent> | null = null;

  private _availableLocales: ILocaleMap = {};

  /** Reference to the currently opened keyboard at *any* level. */
  private get _openedKeyboardRef(): KeyboardRef<KeyboardComponent> | null {
    const parent = this._parentKeyboard;
    return parent ? parent._openedKeyboardRef : this._keyboardRefAtThisLevel;
  }

  private set _openedKeyboardRef(value: KeyboardRef<KeyboardComponent>) {
  ...
  constructor(private _overlay: Overlay,
              ...
              @Inject(KEYBOARD_LAYOUTS) private _layouts: IKeyboardLayouts,
   ...
   */
  openFromComponent(layoutOrLocale: string, config: KeyboardConfig): KeyboardRef<KeyboardComponent> {
    const keyboardRef: KeyboardRef<KeyboardComponent> = this._attachKeyboardContent(config);
   ...
   */
  open(layoutOrLocale: string = this._defaultLocale, config: KeyboardConfig = {}): KeyboardRef<KeyboardComponent> {
   ...
   */
  private _attachKeyboardContainer(overlayRef: OverlayRef, config: KeyboardConfig): KeyboardContainerComponent {
    const containerPortal = new ComponentPortal(KeyboardContainerComponent, config.viewContainerRef);
   ...
   */
  private _attachKeyboardContent(config: KeyboardConfig): KeyboardRef<KeyboardComponent> {
